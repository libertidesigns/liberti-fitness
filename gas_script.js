...
function doGet(e) {
  ...
  try {
    if (p.action === 'read') {
      result = readData(p.type);
    } else if (p.action === 'write') {
      result = writeData(p.type, decodeURIComponent(p.data || '[]'));
    } else {
      result = { ok: false, error: 'Neznama akce' };
    }
  } catch(err) {
...
// doPost handles large data from Android Chrome (avoids URL length limit)
function doPost(e) {
  var p = e.parameter;
  var callback = p.callback || 'cb';
  var result;
  try {
    var action = p.action;
    var type = p.type;
    // Data comes in POST body as plain text JSON
    var dataStr = '';
    if (e.postData && e.postData.contents) {
      dataStr = e.postData.contents;
    } else if (p.data) {
      dataStr = decodeURIComponent(p.data);
    }
    if (action === 'write') {
      result = writeData(type, dataStr);
    } else if (action === 'read') {
